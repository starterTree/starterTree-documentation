...
1.    `name_of_server` type: key  :fontawesome-solid-exclamation:obligatory
      ...
      gegegegeg
      ```
3.    `user@host` type: string , you can add same extra argument of open ssh command like `root@x.X.X.X -t sudo ssh user@X.X.X.X -t` :fontawesome-solid-exclamation:obligatory
